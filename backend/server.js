...
app.post("/api/tblstudent", (req, res) => {
  ...
  const query =
    "INSERT INTO tblstudent (firstname, lastname, gender, dob, address) VALUES (?, ?, ?, ?, ?)";
...
app.get('/api/provinces', (req, res) => {
  db.query('SELECT ID, Name FROM tblprovinces', (err, results) => {
    if (err) {
      console.error("Error fetching provinces:", err);
      return res.status(500).json({ message: "Error fetching provinces", error: err.message });
    }
    res.json(results); // Send back the result
  });
});
